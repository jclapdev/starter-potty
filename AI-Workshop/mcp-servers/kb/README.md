...
**Current state:** the six active agents are all vault-meta agents (wrap-up, vault-maintenance, backlink-scan, handoff-writer, skill-eval-runner, agent-detector) — none debug external tools, so none is a correct home for this fast-path yet. Wire it in when a tool-problem-solving agent exists (e.g. a comic-generator agent for the ComfyUI pipeline).
